...
export default function ConfirmModal({
  ...
  const getIcon = () => {
    switch (type) {
      ...
      case "duplicate":
        return <DocumentDuplicateIcon className="h-6 w-6 text-green-600" />;
      case "info":
            ...
            >
              <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
                ...
                <div className="flex space-x-3 justify-end">
                  ...
                  <button
                    type="button"
                    className={`inline-flex justify-center rounded-lg px-4 py-2 text-sm font-medium text-white focus:outline-none focus:ring-2 focus:ring-offset-2 transition-colors ${getButtonColors()}`}
                    onClick={() => {
                      onConfirm();
                      onClose();
                    }}
